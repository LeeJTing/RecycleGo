...
class _AppealPageState extends State<AppealPage> {
  ...
  Future<void> _loadData() async {
    try {
      // 1️⃣ Get submission (Note: Check if your table is recycle_submissions or recyclingsubmission!)
      final submission = await supabase
          .from('recycle_submissions') // I updated this to match your Model file!
          .select()
  ...
  Widget _buildSubmissionInfo(AppColors theme) {
    if (submissionData == null) return const SizedBox();

    final status = submissionData!['status']?.toString() ?? 'Unknown';
